...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		ChessPiece movedPiece = (ChessPiece)board.piece(target); //a peça que se moveu é a peça que foi colocada no destino
	...
	public ChessPiece replacePromotedPiece(String type) {
		if (promoted == null) { //tem que ter uma peça lá indicando para ser removida se não vai dar errado
	...
	private ChessPiece newPiece(String type, Color color) { //esse método auxiliar é para instanciar a peça específica para o jogo
	...
	private Piece makeMove(Position source, Position target) {
		...
		if(p instanceof Pawn) {
			if(source.getColumn() != target.getColumn() && capturedPiece == null) { //ta testando se o peão andou na diagonal e não capturou peça significa que é um en passant
				Position pawnPosition;
				if (p.getColor() == Color.WHITE) {
					pawnPosition = new Position(target.getRow() + 1, target.getColumn()); //posição do peão que tem que ser capturado
				}
				else {
					pawnPosition = new Position(target.getRow() - 1, target.getColumn());
				}
				capturedPiece = board.removePiece(pawnPosition);
				capturedPieces.add(capturedPiece); //adicionando a peça removida do tabuleiro na lista
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		if(capturedPiece != null) { //retorna a jogada para o lugar de origem
			board.placePiece(capturedPiece, target); //o algoritmo genérico já retorno a peça capturada
		...
		// #movimento especial en passant
		if(p instanceof Pawn) {
			if(source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) { //ta desfazendo uma jogada se a peça for vulneravel ao en passant
				ChessPiece pawn = (ChessPiece)board.removePiece(target);//tirou do lugar errado da peça
				Position pawnPosition;
				if (p.getColor() == Color.WHITE) {
					pawnPosition = new Position(3, target.getColumn()); //posição do peão que tem que ser capturado
				}
				else {
					pawnPosition = new Position(4, target.getColumn()); // se a peça que capturou foi a preta, vai ter que devolver a peça branca capturada para linha 4
				}
